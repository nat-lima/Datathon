...
Qual sua experiência com projetos na área de atuação?"""
    ...
    assert "Quais são suas principais habilidades técnicas?" in perguntas
    assert titulo == "Desenvolvedor Backend"
    assert objetivo == "Desenvolver APIs escaláveis"
    assert "João Silva" in resumo
    assert "Python" in competencias